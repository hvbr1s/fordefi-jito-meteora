...
async function getTokenBalance(
  ...
  // In parsed data, the token balance is in `uiAmountString` as per documentation
...
async function main() {
  ...
  // Convert TRUMP holdings to USDC value
  ...
  const deltaForJupiter = Math.floor(absDelta * 1_000_000);  // 6 decimals for TRUMP
  ...
  // Create JSON body (for our FORDEFI signer)
  const jsonBody = {
    vault_id: VAULT_ID,
